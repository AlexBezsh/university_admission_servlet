...
public class JDBCFacultyDao extends JDBCDao implements FacultyDao {
    ...
    public List<Faculty> findAll(PageInfoDTO pageInfo) {
        String getFacultiesCountQuery = "SELECT COUNT(id) as count FROM faculty";
        String getFacultiesQuery =
                "SELECT faculty.id AS f_id, " +
                    "faculty.name_en AS f_name_en, " +
                    "faculty.name_ua AS f_name_ua, " +
                    "faculty.status AS f_status, " +
        ...
        try (PreparedStatement getFacultiesStmt = connection.prepareStatement(getFacultiesQuery);
            ...
            ResultSet resultSet = getFacultiesCountStmt.executeQuery();
            resultSet.next();
            int count = resultSet.getInt("count");
            if (count % pageInfo.getSize() == 0) {
                pageInfo.setTotalPages(count / pageInfo.getSize());
            } else {
                pageInfo.setTotalPages(count / pageInfo.getSize() + 1);
            }

            return findFaculties(getFacultiesStmt);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
